...
/*mmu建立页表原理，首先是mmu虚拟地址通过一级页表转换的物理地址的原理，
虚拟地址取出前12位做为偏移地址，页表首地址在p15协处理器中存储，通过偏移和首地址得出具体的转换
所以：
1：	在内存中建立页表，取首地址为0x3000 0000，并保存在p15协处理器中
2：	将首地址+偏移地址所处位置中存入具体转换的物理地址值*/
...
#define MMU_SECTION_SIZE    0x00100000
    ...
    virtuladdr = 0;
    physicaladdr = 0;
    //物理地址和虚拟地址的映射表，一条指令的映射大小为1M
    *(mmu_tlb_base + (virtuladdr >> 20)) = (physicaladdr & 0xFFF00000) | \
                                            MMU_SECDESC_WB;
		//*(mmu_tlb_base + (virtuladdr >> 20)):转化表首地址和虚拟偏移地址所指空间
		//(physicaladdr & 0xFFF00000) | MMU_SECDESC_WB: 物理地址1M(=2^20=0x100000)寻址的首地址
